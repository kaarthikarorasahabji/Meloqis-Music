...
class MeloqisReleaseAlertWorker(
    ...
    override suspend fun doWork(): Result {
        return runCatching {
            ...
            val downloadUrl = manifest.getString("downloadUrl").trim()
            val sha256 = manifest.getString("sha256").trim().lowercase()
            if (
                ...
                val notificationPosted = UpdateNotificationHelper.showUpdateNotification(
                    applicationContext,
                    latestVersion,
                    downloadUrl,
                    sha256,
...
object MeloqisReleaseAlerts {
    ...
    private const val FOREGROUND_WORK = "meloqis_official_release_foreground_check"
    ...
    fun checkNow(context: Context) {
        val request = OneTimeWorkRequestBuilder<MeloqisReleaseAlertWorker>()
            .setConstraints(
                Constraints.Builder()
                    .setRequiredNetworkType(NetworkType.CONNECTED)
                    .build(),
