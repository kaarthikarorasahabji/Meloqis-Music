...
object UpdateNotificationHelper {
    ...
    fun showUpdateNotification(
        ...
        val intent = Intent(context, MeloqisUpdateActionReceiver::class.java).apply {
            action = MeloqisUpdateActionReceiver.ACTION_DOWNLOAD_UPDATE
            putExtra(MeloqisUpdateActionReceiver.EXTRA_VERSION, versionName)
            putExtra(MeloqisUpdateActionReceiver.EXTRA_DOWNLOAD_URL, downloadUrl)
            putExtra(MeloqisUpdateActionReceiver.EXTRA_SHA256, sha256)
        }

        val flags = PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
        val pending = PendingIntent.getBroadcast(context, NOTIFICATION_ID, intent, flags)
        ...
        val notif = NotificationCompat.Builder(context, CHANNEL_ID)
            ...
            .setStyle(
                NotificationCompat.BigTextStyle()
                    .bigText("Meloqis $versionName is available from the official Axenora release channel. Tap Update now to download and verify it inside Meloqis.")
            )
